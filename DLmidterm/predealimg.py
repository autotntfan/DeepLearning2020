...
def Deal_With_Img(num,datatype):
    if not os.path.exists("DLmidterm/"+datatype+"dealed/"): 
        os.mkdir("DLmidterm/"+datatype+"dealed/")
    data=scipy.io.loadmat('DLmidterm/'+datatype+".mat") #讀取資料
    x,y,y_onehot=data["x"],data["y"],data["y_onehot"]      #讀取資料
    data=x[0][num]                           #處理第num張圖片
    path="DLmidterm/"+datatype+"dealed/"  #第一次處理完的圖片儲存位置
...
def Deal_With_TestImg(num,data,datatype):
    if not os.path.exists("DLmidterm/"+datatype+"dealed/"): 
        os.mkdir("DLmidterm/"+datatype+"dealed/")
    path="DLmidterm/"+datatype+"dealed/"
